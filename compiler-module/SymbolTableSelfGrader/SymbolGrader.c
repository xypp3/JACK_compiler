...
char *ErrorString(SyntaxErrors e) {
  ...
  case memberDeclarErr:
    return "class member declaration must begin with static, field, "
  ...
  case subroutineDeclarErr:
    return "subrouting declaration must begin with constructor, function, or "
           "method";
...
int t_compiler() {
  ...
  for (int j = 0; j < NumberTestFiles; j++) // for each test file
  {
    ...
    InitCompiler();
    fail = 0;
    char str[128] = strncat("./test-files/", testFiles[j], 128);
    ParserInfo p = compile(str);
    if (p.er != correctInfo[j].er)
      fail = 1;
    else if (correctInfo[j].er != none &&
             (p.tk.tp != correctInfo[j].tk.tp ||
              p.tk.ln != correctInfo[j].tk.ln ||
              (correctInfo[j].er != lexerErr &&
               strcmp(correctInfo[j].tk.lx, p.tk.lx))))
      fail = 1;
    else
      printf("\t$$ Great PASSED :-)\n");
    if (fail) {
