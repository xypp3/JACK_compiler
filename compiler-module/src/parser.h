...
typedef enum {
  ...
  memberDeclarErr,    // class member declaration must begin with static, field,
  ...
  subroutineDeclarErr,  // subrouting declaration must begin with constructor,
                        // function, or method
  ...
  undecIdentifier, // undeclared identifier (e.g. class, subroutine, or
                   // variable)
...
// and a token field (tk) that is set to the token at or near which the error
// was encountered if no errors are encountered er should be set to none
typedef struct {
  SyntaxErrors er;
  Token tk;
...
int InitParser(
    char *file_name); // initialise the parser to parse source code in file_name
